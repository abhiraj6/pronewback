...
app.post('/adda', async (req, res) => {
  ...
  app.put('/updatep/:id', async (req, res) => {
    ...
    try {
      const updatedPizza = await Pizza.findByIdAndUpdate(id, { title, description, imageUrl }, { new: true });
      if (!updatedPizza) {
        return res.status(404).json({ message: 'Pizza not found' });
      }
      res.status(200).json({ message: 'Pizza updated successfully', updatedPizza });
    } catch (error) {
      console.error('Error updating pizza:', error);
      res.status(500).json({ message: 'Error updating pizza. Please try again.' });
    }
  });
